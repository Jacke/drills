...
export const parseLinks = async (pageId: string) => {
  let lastPageBlocks = await notion.blocks.children.list({
    block_id: pageId,
    page_size: 100,
  });
  let accBlocks: Array<GetBlockResponse> = lastPageBlocks.results;
  ...
  const blocks: Array<GetBlockResponse> = accBlocks.flat();
  const urls: string[] = [];
...
const saveLinksToJson = async (fileName: string, links: any[]) => {
  return await writeFile(
    `/Users/stan/Dev/_Side/@Drills/drilllist-server/data/${fileName}.json`,
    jsonToString(links),
  );
};
...
export const extractPageIdsFromRootPage = async () => {
  ...
  while (response.has_more) {
    response = await notion.blocks.children.list({
      block_id: rootPageId,
      start_cursor: response.next_cursor as any,
      page_size: 100,
    });
